...
export default function ScheduleForm({
  ...
  const {
    data: ppcData,
    isLoading: isPpcLoading,
    isError,
  } = useProductScheduleByDateAndLineNo(date, lineNoForFetching, {
  ...
  const updateItem = (index: number, updates: Partial<FormItem>) => {
    ...
    const isQtyChanged =
      updates.qty !== undefined && updates.qty !== oldItem.qty;
    const isBoQtyChanged =
      updates.boQty !== undefined && updates.boQty !== oldItem.boQty;
    ...
    if (isQtyChanged && oldIdentifier) {
      ...
      const subsequentTargetIndices = newItems
        ...
        .filter(({ item }) => {
          ...
          return (
            String(item.machineNo) === String(oldItem.machineNo) &&
      ...
      if (subsequentTargetIndices.length > 0) {
        const baseDist = Math.trunc(
          distributionTotal / subsequentTargetIndices.length,
        );
        const isNegative = distributionTotal < 0;
        let absRemainder =
          Math.abs(distributionTotal) % subsequentTargetIndices.length;
    ...
    const isQtyAffectsBo =
      isQtyChanged && !oldItem.isBuildAch && oldItem.boQty !== null;
    ...
    if ((isQtyAffectsBo || isBoQtyChanged) && oldIdentifier) {
      ...
      if (allItemsForCode.length > 0) {
        ...
        const shifts = Array.from(
          new Set(allItemsForCode.map((x) => Number(x.item.shiftNo))),
        ).sort((a, b) => a - b);
        let currentBO = 0;

        for (let i = 0; i < shifts.length; i++) {
          const shift = shifts[i];
          const shiftItems = allItemsForCode.filter(
            (x) => Number(x.item.shiftNo) === shift,
          );

          if (
            i === 0 ||
            (shift === Number(oldItem.shiftNo) && isBoQtyChanged)
          ) {
        ...
        boQty:
          item.boQty === null || item.boQty === undefined || item.boQty === ""
            ...
            : item.boQty === "F" ||
              ...
              : String(item.boQty),
        remainingBoQty:
          item.remainingBoQty !== null && item.remainingBoQty !== undefined
            ? String(item.remainingBoQty)
            : "0",
          ...
          <div className="space-y-6">
            {!selectedLineId ? (
              <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-lg bg-muted/20 text-muted-foreground">
                ...
                <p className="text-sm">
                  Machines will be automatically displayed based on your
                  selection
                </p>
              </div>
            ) : isPpcLoading ? (
              <div className="space-y-4">
                {Array.from({ length: 2 }).map((_, i) => (
                  <div
                    key={i}
                    className="border rounded-lg p-6 bg-background shadow-sm space-y-4"
                  >
              ...
              <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-lg bg-muted/20 text-muted-foreground">
                ...
                <p className="text-sm">
                  No production data available for this category and date.
                </p>
              </div>
            ) : (
